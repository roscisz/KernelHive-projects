...
//	CL_CHECK(clUnloadCompiler());
	...
	int nThreadsInGroup = 1;// Only MIC atoi(argv[2]);
	...
	int output_buffer_size = WIDTH * HEIGHT * sizeof(int);
	...
	CL_CHECK(clEnqueueReadBuffer(queue, output_buffer, CL_TRUE, 0, output_buffer_size, data, 0, NULL, NULL));
	int x, y;
